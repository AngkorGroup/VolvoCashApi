...
namespace VolvoCash.Domain.MainContext.Aggregates.DealerAgg
{
    public class Dealer : AuditableEntityWithKey<int>
    {
        ...
        {
            var defaultBankAccount = BankAccounts.FirstOrDefault(ba => ba.BankId == bankId && ba.CurrencyId == currencyId && ba.IsDefault);

            if (defaultBankAccount == null)
            {
                defaultBankAccount = BankAccounts.FirstOrDefault(ba => ba.BankId == bankId && ba.CurrencyId == currencyId);
                if (defaultBankAccount == null)
                {
                    defaultBankAccount = BankAccounts.FirstOrDefault(ba => ba.CurrencyId == currencyId);
                }
            }
